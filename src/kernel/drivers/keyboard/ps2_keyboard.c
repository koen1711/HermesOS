...
void ps2_keyboard_initialize() {
    ports_clear_read_buffer(COMMAND_PORT, DATA_PORT);

    // Controller Command OxAE = Enable Keyboard
    ports_write_uint8(COMMAND_PORT, 0xAE);
    // Controller Command 0x20 = Read Controller Command Byte
    ports_write_uint8(COMMAND_PORT, 0x20);
    // Set the status
    const uint8_t status = (ports_read_uint8(DATA_PORT) | 1) & ~0x10;
    // Controller Command 0x60 = Write Controller Command Byte
    ports_write_uint8(COMMAND_PORT, 0x60);
    ports_write_uint8(DATA_PORT, status); // Write the status

    // Controller Command 0xAA = Self Test
    ports_write_uint8(COMMAND_PORT, 0xAA);
    if (ports_read_uint8(DATA_PORT) != 0x55)
        return; //TODO: Handle error
}

void ps2_keyboard_interrupt_handler() {
    char charKey = '\0';
    uint8_t key = ports_read_uint8(DATA_PORT);
    ...
    // Modifiers
    const uint8_t mods = (ctrl & MOD_CTRL) | (alt & MOD_ALT) | (shift & MOD_SHIFT);
