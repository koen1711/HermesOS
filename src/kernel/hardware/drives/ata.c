...
#include "../../cfunctions/threading/mutex.h"
...
/*
ata_probe sends an IDENTIFY DEVICE command to the hardware.
If a hardware is connected, it will respond with 512 bytes
of identifying data, described on page 48 of the ATA-3 standard.
If no response comes within the timeout window, we assume
the the hardware is simply not connected.
*/
