...
uint32_t pci_config_read_u32(const uint8_t bus, const uint8_t slot, const uint8_t func, const uint8_t offset) {
	...
	pci_write_u32(PCI_ADDRESS_PORT, address);
	config = pci_read_u32(PCI_VALUE_PORT);
	return config;
}
...
uint16_t pci_config_read_u16(const uint8_t bus, const uint8_t slot, const uint8_t func, const uint8_t offset) {
	...
	pci_write_u32(PCI_ADDRESS_PORT, address);
	config = pci_read_u16(PCI_VALUE_PORT + (offset & 2));
	return config;
}
...
uint8_t pci_config_read_u8(const uint8_t bus, const uint8_t slot, const uint8_t func, const uint8_t offset) {
	...
	pci_write_u32(PCI_ADDRESS_PORT, address);
	config = pci_read_u8(PCI_VALUE_PORT + (offset & 3));
	return config;
}
...
void pci_initialize()
{
	...
	for (int i = 0; i < result->count; i++) {
		...
		vga_text_write_string("PCI: ");
		vga_text_write_string(vendor_name);
		vga_text_newline();
	}
}
...
void pci_scan_func(const int type, const int bus, const int slot, const int func, struct scan_result *result) {
...
void pci_scan_slot(const int type, const int bus, const int slot, struct scan_result *result) {
...
void pci_scan_bus(const int type, const int bus, struct scan_result *result) {
	for (int slot = 0; slot < PCI_NUM_DEVICES; ++slot) {
		pci_scan_slot(type, bus, slot, result);
	}
}

struct scan_result* pci_scan(const int type) {
	struct scan_result* result = malloc(sizeof(struct scan_result));
	result->count = 0;
