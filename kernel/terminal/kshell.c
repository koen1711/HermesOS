...
#include "../hardware/drives/bcache.h"
#include "../cfunctions/io/printf.h"
	...
	if(dev) {
		...
		if(fs) {
			...
			if(v) {
				...
				if(d) {
                    printf("kshell_mount: d is not null\n");
					current->ktable[KNO_STDDIR] = kobject_create_dir(d);
                    printf("kshell_mount: kobject_create_dir done\n");
					return 0;
				} else {
		...
		printf("mount: couldn't open hardware %s unit %d\n",devname,unit);
		return -1;
	}
...
static int kshell_cat(const char *path)
{
    ...
    if(d) {
        int buffer_length = 4096;
        char *buffer = page_alloc(0);
        int length = fs_dirent_read(d, buffer, buffer_length, 0);
        int offset = 0;
        if (length <= 0) {
        ...
        while (buffer_length == length) {
            printf("%s\n", buffer);
            offset += length;
            length = fs_dirent_read(d, buffer, buffer_length, offset);
        }
        printf("%s\n", buffer);
        fs_dirent_close(d);
        page_free(buffer);
...
static int kshell_execute(int argc, const char **argv)
{
	...
	if(!strcmp(cmd, "start")) {
		...
		if(argc==4) {
			...
			printf("mount: requires hardware, unit, and fs type\n");
		}
			...
			if(str2int(argv[2], &unit)) {
				...
				if(f) {
					...
					if(d) {
						...
						printf("couldn't open hardware %s unit %d\n",argv[1],unit);
					}
				} else {
		...
		printf("Kernel Shell Commands:\ncat <path>\nrun <path> <args>\nstart <path> <args>\nkill <pid>\nreap <pid>\nwait\nlist\nautomount\nmount <hardware> <unit> <fstype>\numount\nformat <hardware> <unit><fstype>\ninstall <srcunit> <dstunit>\nmkdir <path>\nremove <path>time\nbcache_stats\nbcache_flush\nreboot\nhelp\n\n");
	} else {
