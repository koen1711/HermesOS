...
/*
If the hardware driver specifies a non-zero default multiplier,
then save it in this hardware instance.  It gives the effect of
...
*/
    ...
    printf("device_create: d->multiplier = %d\n", d->multiplier);
	return d;
}
...
int device_read(struct device *d, void *data, int size, int offset)
{
	int status;
	if(d->driver->read) { 
		int status = d->driver->read(d->unit,data,size*d->multiplier,offset*d->multiplier);
		if (status) {
